...
import { ClickEvent, FyleField, MappingState, PaginatorPage, XeroField, ZeroStatePage } from 'src/app/core/models/enum/enum.model';
...
export class GenericMappingComponent implements OnInit {
  ...
  private getMappingsAndSetupPage(): void {
    ...
    this.mappingService.getMappingSettings().subscribe((mappingSettingResponse: MappingSettingResponse) => {
      ...
      this.mappingSetting = mappingSetting.length ? mappingSetting[0] : {source_field: FyleField.CATEGORY, destination_field: XeroField.ACCOUNT};
